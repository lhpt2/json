...
/// Both quote styles decode identically -- the grammar's `escaped`
/// production allows `\'` and `\"` in either one, so the only thing that
/// differs between them is which byte terminates the scan. This crate has
/// no `serde_json`-internal `Read`/`StrRead` to borrow the decoding from
/// (see the workspace's `docs/` for the extraction notes on that
/// trade-off), so both styles share this one self-contained scanner.
