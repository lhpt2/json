...
This crate was extracted from a CSON-flavored fork of `serde_json` that
lives alongside it in this repository (see `../` and its
`docs/legacy-value-serde-path.md`), where it started life as an in-tree
module. It has no dependency on that crate, or on any `serde_json`
internals — the only dependency is `serde` itself, for the typed
read/write layer described below.
