...
//!
//! Mirrors `src/value/ser.rs`'s `Serializer { type Ok = Value; .. }`
...
//! this is the "typed value -> tree" half of the round trip; combining it
//! with an *existing* tree's trivia is `Document::merge_from`'s job, not
//! implemented yet (see `CLAUDE.md`'s build order).
...
impl ser::Serializer for Serializer {
    ...
    fn serialize_i64(self, value: i64) -> Result<Node<'static>, ParseError> {
        Ok(number_node(itoa::Buffer::new().format(value).to_owned()))
    }
    ...
    fn serialize_f32(self, value: f32) -> Result<Node<'static>, ParseError> {
        if value.is_finite() {
            Ok(number_node(zmij::Buffer::new().format_finite(value).to_owned()))
        } else {
            Ok(node(Value::Null))
        }
    }
    ...
    fn serialize_bytes(self, value: &[u8]) -> Result<Node<'static>, ParseError> {
        let items = value
            .iter()
            .map(|&b| number_node(itoa::Buffer::new().format(b).to_owned()))
            .collect();
