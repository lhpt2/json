...
//! without going through `T`'s usual text round trip -- i.e. without
//! losing the ability to also still hold onto the comments, the way
//! `crate::from_str::<T>` would. The structure mirrors
//! `src/value/de.rs`'s `impl<'de> Deserializer<'de> for &'de Value`
//! closely on purpose (same method bodies, same helper-struct shapes),
//! adapted to this module's `Value`/`Node`/`Entry` types instead of
